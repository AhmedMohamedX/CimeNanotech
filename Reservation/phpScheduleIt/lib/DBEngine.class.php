...
class DBEngine {
 ...
 /**
    * Return all reservations associated with a user- using email, instead of userID  version!!!!
    * @param string $id now expects username
    * @param string $order the order for the return results
    * @param string $vert the vertical sorting order
    * @param bool $include_participating if this should include the reservations where the user is only particpating
    * @return array of reservation data
    */
    function get_user_reservations_mod($id, $order, $vert, $include_participating = false) 
    {
        $return = array();
		...
		$query = 'SELECT res.*, resusers.*, rs.name, rs.rphone, rs.location FROM '
                    . $this->get_table('reservations') . ' as res INNER JOIN '
                    . $this->get_table('resources') . ' as rs ON rs.machid=res.machid INNER JOIN '
                    . $this->get_table('reservation_users') . ' as resusers ON resusers.resid=res.resid INNER JOIN '
					. $this->get_table('login') . ' as login ON login.memberid=resusers.memberid'
                    . ' WHERE login.email=?'
                    . ' AND (res.start_date>=? OR (res.start_date<=? AND res.end_date>=?))'
                    . ' AND res.is_blackout <> 1'
                    . (!$include_participating ? ' AND owner = 1' : ' AND invited = 0')
    ...
    /**
    * Return all reservations associated with a resource (copied from get_user_reservations() above).
    * @param string $rs_name resource name
    * @param string $order the order for the return results
    * @param string $vert the vertical sorting order
    * @param bool $include_participating if this should include the reservations where the user is only particpating
    * @return array of reservation data
    */
    function get_resource_reservations($rs_name, $order, $vert) 
    {
        $return = array();
		...
		$query = 'SELECT res.*, resusers.*, rs.name, rs.rphone, rs.location, login.email, login.fname, login.lname FROM '
                    . $this->get_table('reservations') . ' as res INNER JOIN '
                    . $this->get_table('resources') . ' as rs ON rs.machid=res.machid INNER JOIN '
                    . $this->get_table('reservation_users') . ' as resusers ON resusers.resid=res.resid INNER JOIN '
 					. $this->get_table('login') . ' as login ON login.memberid=resusers.memberid'
        ...
        $values = array($rs_name, mktime(0,0,0), mktime(0,0,0), mktime(0,0,0));
        ...
        if ($result->numRows() <= 0) 
        {
            $this->err_msg = "This system has no scheduled reservations";
            return false;
        }
